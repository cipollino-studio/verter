...
#[test]
fn hello_world() {
    let mut file = File::open("test/hello.verter", Config::default()).unwrap();
    ...
    std::fs::remove_file("test/hello.verter").unwrap();
}

#[test]
fn deletion() {
    let mut file = File::open("test/deletion.verter", Config::default()).unwrap();
    ...
    std::fs::remove_file("test/deletion.verter").unwrap();
}

#[test]
fn truncation() {
    let mut file = File::open("test/truncation.verter", Config::default()).unwrap();
    file.write_root(&vec![0xAE; 2000]).unwrap();
    file.write_root(&vec![0xBA; 200]).unwrap();
    drop(file);

    let file_size = std::fs::metadata("test/truncation.verter").unwrap().len();

    let mut file = File::open("test/truncation.verter", Config::default()).unwrap();
    file.alloc().unwrap();
    drop(file);

    let new_file_size = std::fs::metadata("test/truncation.verter").unwrap().len();

    assert_eq!(file_size, new_file_size);

    std::fs::remove_file("test/truncation.verter").unwrap();
} 

#[test]
fn magic_bytes() {
    let file = File::open("test/magic_bytes.verter", Config {
        magic_bytes: b"Magic1",
        ..Config::default()
    }).unwrap();
    drop(file);

    match File::open("test/magic_bytes.verter", Config {
    ...
    std::fs::remove_file("test/magic_bytes.verter").unwrap();
}

#[test]
fn invalid_pointer() {
    let mut file = File::open("test/invalid_pointer.verter", Config::default()).unwrap();
    ...
    std::fs::remove_file("test/invalid_pointer.verter").unwrap();
}

#[test]
fn extension() {
    let mut file = File::open("test/extension.verter", Config::default()).unwrap();
    let alloc = file.alloc().unwrap();
    drop(file);
    ...
    std::fs::remove_file("test/extension.verter").unwrap();
}
